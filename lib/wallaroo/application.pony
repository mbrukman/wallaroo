...
class val Application
  ...
  new val create(env: Env, name: String) =>
    _env = env
    _app_name = name

  fun start(pipeline: BasicPipeline val) =>
    if pipeline.is_finished() then
      Startup(env, this, _app_name, pipeline)
    else
...
primitive Wallaroo
  ...
  =>
    Pipeline[In](0, pipeline_name, source_config)
...
class Pipeline[Out: Any val] is BasicPipeline
  ...
  =>
    _pipeline_id = p_id
    _name = n
    _stages = Dag[(RunnerBuilder | SinkBuilder)]
  ...
  fun ref to_sink(sink_information: SinkConfig[Out]): Pipeline[Next] =>
    if not _finished then
